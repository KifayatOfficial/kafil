...
- **§11 — In-app notification inbox (read side):** the notification WRITE pipeline (`notifications.service.ts` — push/SMS/WA + an in-app row per send) had no matching READ path. Added `notification-inbox.service` (keyset-paginated `list`, `unreadCount` over the existing `@@index([userId, readAt])`, userId-scoped `markRead`/`markAllRead`) + routes (`GET /api/notifications`, `GET /api/notifications/unread-count`, `POST /api/notifications/read-all`, `POST /api/notifications/:id/read` — all inherently idempotent, no schema change). Mobile: `useNotificationCount` (refresh on notification-correlated SSE hints), a `NotificationsScreen` inbox (FlashList infinite scroll, StatefulView, optimistic tap/mark-all read), a `<Badge>` on the **'You' bottom tab** (the §1.2 "badge counts on tabs" ask) + a Notifications row in the You hub. Backend fully tested (api 215, +5); mobile typecheck-only.
